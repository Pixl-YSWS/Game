...
const SESSION_REFRESH_AFTER_MS = 1000 * 60 * 60 * 24;
...
export function setupAuth(db: Database): AuthModule {
  ...
  const selectHackatimeKey = db.query<
    { hackatime_key: string | null },
    [string]
  ...
  router.get("/login", (_req, res) => {
    if (!authConfigured()) {
      res
        .status(500)
        .send(
          "Auth not configured: set HACKCLUB_CLIENT_ID / HACKCLUB_CLIENT_SECRET.",
        );
      return;
    }
    sweepStates();
  ...
  router.get("/callback", async (req, res) => {
    ...
    if (!state || !pendingStates.delete(state)) {
      res
        .status(400)
        .send("Invalid or expired auth state. Try logging in again.");
      return;
    }
    if (!code) {
  ...
  function verifySession(token: string): Account | null {
    ...
    if (
      ...
      SESSION_TTL_MS - SESSION_REFRESH_AFTER_MS
    ) {
